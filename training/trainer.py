...
def main(argv, stdout=sys.stdout, stderr=sys.stderr) -> int:
    arg_parser = argparse.ArgumentParser(description="Train QAOA predictor model")
    arg_parser.add_argument("-m", "--model", type=str, choices=[enum.value for enum in ModelType], required=True, help="Model type")
    arg_parser.add_argument("-d", "--dataset_name", type=str, default="ATLAS", help="Dataset name")
    arg_parser.add_argument("-s", "--dataset_size", type=int, help="Dataset size (this will affect the dataset used)")
    ...
    dataset = load_dataset_by_name(name=args.dataset_name, dataset_size=args.dataset_size, n_layers=args.n_layers)
    data_loader = create_dataloader(dataset, batch_size=args.batch_size)
    train(model=model, data_loader=data_loader, epochs=args.epochs, lr=args.lr, split_ratio=args.split_ratio)
    return 0
