...
def split_dataloader(
    data_loader: DataLoader, split_ratio: float
) -> tuple[DataLoader, DataLoader]:
    assert data_loader.batch_size is not None
    split = int(len(data_loader) * split_ratio)
    train_data_loader = DataLoader(
        data_loader.dataset[:split], batch_size=data_loader.batch_size, shuffle=True
    )
...
def run_epoch(
    ...
    train_dataloader, test_dataloader = split_dataloader(data_loader, split_ratio)
    train_epoch_result = inner_epoch(
        model=model, data_loader=train_dataloader, train=True, optimizer=optimizer
    )
    test_epoch_result = inner_epoch(
        model=model, data_loader=test_dataloader, train=False
    )
...
# def split_dataset(dataset, split: float = 0.8) -> tuple[tuple[list[nx.Graph], list[QuantumCircuit], np.ndarray], tuple[list[nx.Graph], list[QuantumCircuit], np.ndarray]]:
#     graphs, qaoa_circuits, optimal_params = dataset
#     split = int(len(qaoa_circuits) * split)
#     print(f"Splitting dataset into {split} training samples and {len(graphs) - split} testing samples")
